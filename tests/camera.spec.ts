...
test('a camera survives a save and an open, at schema 10', async ({ editor }, testInfo) => {
  await editor.clearScene();
  await editor.addObject('Rectangle');
  await editor.deselect();
  ...
  // happens to a file — the guides, scenes and physics precedent. A camera
  // rides in on `scenes`, which the parser passes through verbatim, so a build
